...
void Time::setTime(int h, int m, int s){
    ...
    setSecond(s); //set private fieldd second
}
...
string Time::toUniversalString() const {
    ...
    output << setfill('0') << setw(2) << getHour() << ":" << setw(2) << getMinuteU() << ":" << setw(2) << getSecond();
    return output.str();
}
...
string Time::toStandardString() const {
    ostringstream output;
    output << ((getHour() == 0 || getHour() == 12) ? 12 : getHour() % 12) << ":" << setfill('0') << setw(2) << getMMinute() << ":" << setw(2) 
